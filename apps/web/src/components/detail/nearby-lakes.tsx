...
export function NearbyLakes({ lakes }: { lakes: Nearby[] }) {
  ...
  return (
    <div className="rounded-4xl bg-white/80 backdrop-blur-md border border-white/60 p-5 shadow-[0_8px_30px_rgba(14,165,233,0.08)]">
      <div className="font-semibold text-deep">Nearby lakes</div>
      <div className="text-xs text-slate-500 mt-0.5">Within 800 km</div>
        ...
        {lakes.map((l) => {
          ...
          return (
            <li key={l.id}>
              <Link
                href={`/lake/${l.slug}`}
                className="group flex items-center gap-2.5 rounded-2xl px-2 py-1.5 hover:bg-water-50/70 transition"
              >
                <span
                ...
                <ChevronRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-water-500 transition" />
              </Link>
            </li>
          );
        })}
